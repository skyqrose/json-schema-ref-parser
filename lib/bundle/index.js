...
function crawl (parent, key, path, pathFromRoot, indirections, inventory, $refs, options, customRoots) {
  ...
  if (obj && typeof obj === "object" && !ArrayBuffer.isView(obj)) {
    if ($Ref.isAllowed$Ref(obj)) {
      ...
      let defaultRoot = getDefaultRoot(pathFromRoot, options.bundle.defaultRoot);
      let keys = Object.keys(obj)
        .sort((a, b) => {
          if (defaultRoot !== null) {
            let aDefinitionsIndex = `${pathFromRoot}/${a}`.lastIndexOf(defaultRoot);
            let bDefinitionsIndex = `${pathFromRoot}/${b}`.lastIndexOf(defaultRoot);
 ...
 * @param {string|RootFunction} defaultRoot - The default root to optimize for.
 * @returns {string|null}
 */
function getDefaultRoot (pathFromRoot, defaultRoot) {
  return typeof defaultRoot === "string" ?
    defaultRoot
    : typeof defaultRoot === "function" ?
      defaultRoot(pathFromRoot)
      : null;
}
...
function remap (schema, inventory, $refs, options, customRoots) {
  ...
  inventory.sort((a, b) => {
    if (a.file !== b.file) {
    ...
    else if (options.bundle.defaultRoot) {
      let aDefaultRoot = getDefaultRoot(a.pathFromRoot, options.bundle.defaultRoot);
      let bDefaultRoot = getDefaultRoot(b.pathFromRoot, options.bundle.defaultRoot);

      if (!aDefaultRoot && !bDefaultRoot) {
        return 0;
      }
      ...
      let aDefinitionsIndex = a.pathFromRoot.lastIndexOf(aDefaultRoot);
      let bDefinitionsIndex = b.pathFromRoot.lastIndexOf(bDefaultRoot);
